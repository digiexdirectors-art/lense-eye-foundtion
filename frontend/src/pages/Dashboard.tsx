...
import { Eye, LogOut, User as UserIcon, Phone, Shield, Stethoscope, Clock, CreditCard, Edit2, Save, X } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      login(data.data, token as string); // Update global state
  ...
  return (
    <div className="dashboard-layout">
      ...
      <main className="main-content">
        {/* Analytics Section (Role-Based: Global for Admin, Personal for Doctors) */}
        {(user.role === 'admin' || user.role === 'doctor' || user.role === 'accountant') && (
            <ClinicAnalytics />
        )}

        <div className="page-header">
          <h2>My Profile Workspace</h2>
                ...
                <>
                  ...
                  {(user.consultationFee !== undefined && user.consultationFee > 0) && (
                    <div className="profile-item">
                      ...
                      <span className="profile-value">₹{user.consultationFee}</span>
                    </div>
                  )}
                </>
              )}
        ...
        {/* If Admin, render Doctor Management Component */}
        {user.role === 'admin' && <AdminDoctorsView />}
